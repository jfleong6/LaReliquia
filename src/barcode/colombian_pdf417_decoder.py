...
from src.barcode.model import CardIdData, DocumentInfo, Location,CardIdData_2
...
class ColombianIdCardPdf417Decoder:
    ...
    def decode(self) -> Optional[CardIdData]:
        ...
        if len(sp[2]) > 8:
            doc_number = sp[2].decode(COLOMBIAN_PDF417_CODING)[8:18]
        ...
        return CardIdData_2(
            Nombre=f"{first_name} {middle_name} {last_name} {second_last_name}",
            Fecha_nacimiento=f'{anno_nacimiento}-{mes_nacimiento}-{dia_nacimiento}',
            Cedula=doc_number,
        )
